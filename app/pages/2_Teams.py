...
from api.agent import MyTeam, AgentAPI
...
st.write(config['teams'])        

team_name=st.selectbox('Team',options=map(lambda x:x['name'],config['teams']))
team=find(config['teams'],lambda x:x["name"]==team_name)
...
with st.sidebar:
    ...
    st.title('SideBar')
    # st.write(state.events[-1] if len(state.events) else '')
